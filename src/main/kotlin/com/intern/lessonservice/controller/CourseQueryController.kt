...
class CourseQueryController(val courseQueryService: CourseQueryService) {
    ...
    fun getCourseByCourseId(@PathVariable(value="id_course") id_course : Long): ResponseEntity<Course>? {
        return courseQueryService.findByCourseId(id_course)
    }

    @GetMapping("/course/id_teacher={id_teacher}")
    fun getCourseByTeacherId(@PathVariable(value = "id_teacher") id_teacher: Long): MutableList<Course> {
